	...
	$featured = venezuela_blog_get_featured_posts();
	...
	<?php foreach ( $featured as $post ) : setup_postdata( $post ); ?>
		...
		<article id="post-<?php the_ID(); ?>" class="mw-100 ba b1 b--black-20 br2 ma2 ma3-ns bg-white card-shadow">
				...
				<div class="f6 black-70 flex justify-between items-start pt3">
					...
					<?php if ( function_exists( 'get_coauthors' ) ) {
						...
						if ($coauthors[0]->display_name == 'wordpress') {
							...
							<div class="flex">
								<div>
									<?php echo coauthors_get_avatar( $coauthors[0], 50 ); ?>
								</div>
